...
export function tooltip(target, tooltip) {
    ...
    const tooltip_box = ce({
        element: "div",
        className: "tooltip bg-[rgba(0,0,0,0.95)] min-w-72 text-white break-keep absolute hidden z-50 top-y-full lg:top-0 " + css.xCenter,
        append: cover,
    });
    ...
    element_value.map((v, i) => {
        ...
        if (typeof v.value === "object") {
            ...
            switch (v.type) {
                case "ItemTitle": style = ``;
                    break;
                case "SymbolString": style = 'flex flex-wrap';
                    break;
                default: style = ``;
            }
            ...
            if (v.type !== "SymbolString")
                ...
                ce({ element: 'p', className: `mb-2 `, inner: v.value.titleStr, append: inner_objcet });
                let removeText = ['&tdc_kind ', '&tdc_smart', '&tdc_charm', '&tdc_courage'];
                let t = removeText.reduce((a, c, i) => {
                    a = a.replace(c, '');
                    return a;
                }, v.value.contentStr)
                ce({ element: 'p', className: `w-full`, inner: t, append: inner_objcet });
            ...
            switch (v.type) {
                case "NameTagBox":
                    ...
                    break;
                case "MultiTextBox": v.value = v.value.split('|')[1];
                    style = "text-right";
                    break;
            }
            ce({
...
export function equipmentTooltip(target, tooltip) {
    ...
    const tooltipcover = ce({
        className: "tooltip-cover bg-[rgba(0,0,0,0.95)] min-w-72 text-white break-keep absolute group-hover/equi:block hidden z-50 top-full lg:top-0",
        append: target,
    })
}
/**
...
export function tooltip2(target) {
    ...
    const tooltipcover = ce({
        className: "tooltip2 w-92 pt-4 pb-4 pl-2 pr-4 hidden group-hover/tooltip2:block absolute left-[80px] bg-white rounded-[20px] z-20",
        append: target.parentNode
    })
    tooltipcover.append(target);
}

export const title = (text) => `<h2 class="title-h2 ${css.big_title}">${text}</h2>`;
export const title2 = (text, addClass) => `<h3 class="${css.small_title} ${addClass}">${text}</h3>`;
export const label = (label, value, style) => `<b class="${style}">${label}</b> ${value}`;
